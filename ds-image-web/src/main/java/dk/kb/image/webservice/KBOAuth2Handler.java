...
public class KBOAuth2Handler {
    ...
    private KBOAuth2Handler() {
        ...
        if (baseurl == null && mode != MODE.OFFLINE) {
            log.warn("OAuth-enabled endpoints will fail: No security.baseurl defined and security.mode='{}'", mode);
        }

        realms = new HashSet<>(conf.getList("realms", Collections.emptyList()));
        if (realms.isEmpty() && mode != MODE.OFFLINE) {
            log.warn("OAuth-enabled endpoints will fail: No .security.realms defined and security.mode='{}'", mode);
        }
